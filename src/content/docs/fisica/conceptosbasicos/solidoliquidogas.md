...
1.  **Ley de Boyle (P vs V):** Inversamente proporcionales ($P \propto 1/V$). Si comprimes un gas, la presión sube.
2.  **Ley de Charles (V vs T):** Directamente proporcionales ($V \propto T$). Si calientas un gas, se expande.
3.  **Ley de Avogadro (V vs n):** Directamente proporcionales ($V \propto n$). A más moles, más volumen.
...
$$P \cdot V = \left( \frac{m}{M} \right) R \cdot T \implies P \cdot M = \left( \frac{m}{V} \right) R \cdot T$$
$$\text{Densidad } (\rho) = \frac{P \cdot M}{R \cdot T}$$
